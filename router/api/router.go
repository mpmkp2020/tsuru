...
func headersFromConfig(config router.ConfigGetter) (http.Header, error) {
	...
	if headers != nil {
		headersList, isList := headers.([]interface{})
		headersHash, isMapValue := headers.(map[interface{}]interface{})
		if !isMapValue && !isList {
			return nil, errors.Errorf("invalid header configuration: %v", headers)
		}

		for i, possibleHeader := range headersList {
			h, ok := possibleHeader.(map[interface{}]interface{})
			if !ok {
				return nil, errors.Errorf("invalid header configuration at position: %d. Expected map got %v", i, possibleHeader)
			}
			subConfigGetter := router.ConfigGetterFromUnconvertedEntries(h)

			hName, err := subConfigGetter.GetString("name")
			if err != nil {
				return nil, err
			}

			hValue, err := subConfigGetter.GetString("value")
			if err != nil {
				return nil, err
			}

			headerResult.Add(hName, hValue)
		}

		for k, v := range headersHash {
			v, okV := v.(string)
			if !okV {
			...
			headerResult.Add(fmt.Sprint(k), value)
		}
	}
